...
int main(int argc, char **argv)
{
    ...
    /*
     * ORCACV
     * here we test ENN, which not use ground thruth file
     */
    std::cout << "Loading groundtruth from " << opt.path_base << std::endl;
    ...
    // Load Queries
    //==============
    std::cout << "Loading queries from " << opt.path_base << std::endl;
    std::vector<float> massQ(opt.nq * opt.d);
    {
        std::ifstream query_input(opt.path_base, std::ios::binary);
