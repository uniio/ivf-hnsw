...
template<typename T>
int Index_DB::WriteBaseVector(char *table_nm, size_t vec_id, std::vector<T> &ivec) {
    int rc = 0;
    size_t sz = ivec.size();
...
template<typename T>
int Index_DB::WriteVector(char *table_nm, char *col0, char *col1, std::vector<T> &ivec) {
    int rc = 0;
    size_t sz = ivec.size();
    size_t dsize = sz * sizeof(T);
