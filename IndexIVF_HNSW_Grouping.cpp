   ...
   int IndexIVF_HNSW_Grouping::delete_vid_v2(const float *x, idx_t vid)
    ...
    int IndexIVF_HNSW_Grouping::get_batchs_attr() {
        ...
        for (auto i = 0; i < sz; i++) {
            ...
            // 'vector' dim bytes
            size_t rec_size   = sizeof(uint32_t) + d * sizeof(uint8_t);
    ...
    int IndexIVF_HNSW_Grouping::get_batchs_attr_ex() {
        int    rc;
        size_t sz;
    ...
    int IndexIVF_HNSW_Grouping::get_vec_id(const char* vec_path, size_t vec_no, uint32_t& vec_id) {
        ...
        int           rc      = 0;
        size_t        len_rec = sizeof(uint32_t) + d * sizeof(uint8_t);
    ...
    int IndexIVF_HNSW_Grouping::get_vec_id_ex(const char* vec_path, size_t vec_no, uint32_t& vec_id) {
        std::ifstream fs_input;
        fs_input.exceptions(~fs_input.goodbit);
        int           rc      = 0;
    ...
    int IndexIVF_HNSW_Grouping::search(size_t k, const float *query, std::vector<size_t>& id_vectors) {
        ...
        for (int di = k - 1; di >= 0; di--) {
            ...
            rc = get_vec_id_ex(path_batch, vec_no, vec_id);
            if (rc == -1) {
...
#pragma omp parallel for
    ...
    int IndexIVF_HNSW_Grouping::add_queue_to_index_ex(const float* batch,
                                                      const idx_t* idx_batch,
                                                      const size_t* vid,
                                                      size_t vec_size) {
        int rc;
        StopW stopw = StopW();
